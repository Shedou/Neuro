...
# Settings:
export COMMANDLINE_ARGS="--skip-prepare-environment --skip-version-check --no-download-sd-model --skip-torch-cuda-test --no-half --no-half-vae --use-cpu all"


# Don't change!
path_to_script="$( dirname "$(readlink -f "$0")")"
webui_folder="webui1101"
export TORCH_HOME="$path_to_script/$webui_folder/models/ControlNet-fking_torch_cache_dependencies"
export HF_HOME="$path_to_script/cache/huggingface"
